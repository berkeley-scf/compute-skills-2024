...
```bash
$ pwd
/home/jovyan/compute-skills-2026
```
...
$ cd units
$ pwd
/home/jovyan/compute-skills-2026/units
```

We could also navigate through nested subdirectories. For example, after going back to our home directory (using `cd` alone will do this), let's go to the `units` subdirectory of the `compute-skills-2026` subdirectory. The `/` is a separate character that distinguishes the nested subdirectories.

```bash
$ cd
$ cd compute-skills-2026/units
```
...
$ cd ../assets
$ pwd
/home/jovyan/compute-skills-2026/assets
```
...
```bash
$ cd /home/jovyan/compute-skills-2026/units 
$ pwd
/home/jovyan/compute-skills-2026/units
```
...
For example, if we just consider the `compute-skills-2026` directory, we can see the tree structure using `tree` (not available on DataHub):

```bash
$ cd /home/jovyan/compute-skills-2026
$ tree
```
...
The dot (`.`) means "this directory", so the top of the tree here is the `compute-skills-2026` directory itself, within which there are various subdirectories. Then within each of these are files and further subdirectories.
...
drwxr-xr-x 7 jovyan jovyan     4096 Aug  1 15:45  compute-skills-2026
...
$ ls  # Everything after the # is ignored
 compute-skills-2026                          test-jh           Untitled2.ipynb
...
The first should allow you to get `compute-skills-2026` and the second `echo`.
