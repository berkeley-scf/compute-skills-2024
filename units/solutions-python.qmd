...
pow(-1, 0.5)
(-1)**0.5
# sqrt(-1)  # fails for negative numbers
...
# State-based interface (objects created behind the scenes)

import matplotlib.pyplot as plt
plt.plot(gdpPercap, lifeExp, 'bo')
plt.show()

plt.plot(np.log(gdpPercap), lifeExp, 'bo')
plt.xlabel('per capita GDP (log dollars)')
plt.show()

# OOP interface (you interact with the plot objects)
...
```{python}
def count_vowels(x, y_is_vowel=False):
    vowels = 'aeiouAEIOU'
    if y_is_vowel:
        vowels = vowels + 'yY'
    count = 0
