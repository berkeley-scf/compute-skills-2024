...
::: {.callout-tip }
## matplotlib objects
Note that when we made the basic plot in the Software Carpentry material we were (indirectly) interacting with matplotlib objects present in the background, with matplotlib having the concept of the "active" plot (i.e., the current state).

When we made the multi-panel figure we were using an object-oriented interface and directly interacting with the figure (`fig`) and axes (`axes1`, `axes2`, `axes3`) objects. 
...
Finally, we can use `map` to iterate as well.

```{python}
#| eval: true
vals = [0, 5, 7]
result = map(lambda x: x**2, vals)  # a lazy iterator
list(result) # actual result
```
...
Write a function that will count vowels in a character string (i.e., using code from the exercise of the previous section). Allow the user to specify that "y" should count as a vowel.
...
::: {.callout-warning}
One can't use `and` and `or` in vectorized (e.g., numpy) code. Instead we need `&` and `|`.
:::
